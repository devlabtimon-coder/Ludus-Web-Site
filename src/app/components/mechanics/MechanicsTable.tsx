  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-4 sm:p-6">
      <div className="flex items-center justify-between mb-4 sm:mb-6">
        <h2 className="text-lg sm:text-xl font-bold text-gray-900">Dicionário de Mecânicas</h2>
      </div>
      ...
      <div className="rounded-xl overflow-hidden border border-gray-100">
        <div className="overflow-x-auto scrollbar-hide">
          <table className="w-full min-w-[800px]">
            <thead className="bg-[#F7F8FF]">
              <tr>
                <th className="text-left py-3 sm:py-4 px-4 sm:px-5 text-xs font-bold text-[#31358B] uppercase tracking-wide">Mecânica</th>
                <th className="text-left py-3 sm:py-4 px-4 sm:px-5 text-xs font-bold text-[#31358B] uppercase tracking-wide">Categoria</th>
                <th className="text-left py-3 sm:py-4 px-4 sm:px-5 text-xs font-bold text-[#31358B] uppercase tracking-wide">Jogos Vinculados</th>
                    ...
                    <tr key={mechanic.id} className="border-b border-gray-50 hover:bg-gray-50 transition-colors bg-white">
                      <td className="py-3 sm:py-4 px-4 sm:px-5">
                        <div className="flex items-center gap-3">
                          <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-[#F0F2FF] text-[#31358B] flex items-center justify-center border border-[#31358B]/10 shrink-0">
                            <IconComponent size={18} />
                          </div>
                          <div className="min-w-0">
                            <p className="font-bold text-sm sm:text-base text-gray-900 truncate">{mechanic.namePt}</p>
                            {mechanic.nameEn && <p className="text-xs text-gray-500 truncate">{mechanic.nameEn}</p>}
                          </div>
                        </div>
                      </td>
                      
                      <td className="py-3 sm:py-4 px-4 sm:px-5">
                        <span className="bg-gray-100 text-gray-700 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wide whitespace-nowrap">
                          {mechanic.category}
                        </span>
                      </td>
                      
                      <td className="py-3 sm:py-4 px-4 sm:px-5 max-w-[240px] sm:max-w-[280px]">
                        {mechanic.games && mechanic.games.length > 0 ? (
                          <div className="flex flex-wrap gap-1.5">
                            {mechanic.games.map((game, index) => (
                              <span 
                                key={index}
                                className="bg-[#F0F2FF] text-[#31358B] border border-[#31358B]/20 px-2 py-0.5 rounded-md text-[11px] font-bold whitespace-nowrap"
                              >
                                {game}
                              </span>
                            ))}
        ...
        <div className="bg-gray-50 px-4 sm:px-5 py-3 sm:py-4 border-t border-gray-100">
          <p className="text-xs sm:text-sm text-gray-500 font-medium">Exibindo {mechanics.length} mecânicas</p>
        </div>
      </div>
    </div>
  );
